...
AHORCADO = ['''
    ...
    =========''', '''
     ...
     / \  |
          |
    =========''']
palabras = 'tuplas expresiones operaciones programacion funciones lambda reducir filtrar numpy'.split()
...
def elijeLetra(algunaLetra):
    ...
    while True:
        print ('Adivina una letra, recuerda que la palabra es algo relacionado con python ;)')
        letra = input()
        letra = letra.lower()
        if len(letra) != 1:
...
print ('A P R E N D E   P Y T H O N   C O N   A H O R C A D O  ')
letraIncorrecta = ""
letraCorrecta = ""
...
while True:
    ...
    if letra in palabraSecreta:
        ...
        if letrasEncontradas:
            print ('¡Muy bien! La palabra secreta es "' + palabraSecreta + '"! ¡Has ganado, ERES TODO UN PROGRAMADOR DE PYTHON!')
            finJuego = True
    else:
